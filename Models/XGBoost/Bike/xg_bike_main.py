...
def main():
    ...
    if editor_mode:
        results.to_excel(f"{getPaths('outputVal')}/Validation_Bike_XGBoost_Eval_{current_date}.xlsx")
    elif forecasting:
        results.to_excel(f"{getPaths('outputVal')}/Validation_Bike_XGBoost_Forecasting_{current_date}.xlsx")

    # Evaluation
    save_dir = f"{getPaths('xg_eval')}/Bike"
    all_results = []
